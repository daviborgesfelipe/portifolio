...
function updateContent(imageKey: any) {
  const contentContainer = document.getElementById("contentContainer");
  const content = contentData[imageKey];
...
const imageKeys = Object.keys(contentData);
imageKeys.forEach((key) => {
  const imageElement = document.getElementById(`${key}Image`);
